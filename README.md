...
| `` `{goto:private static:private static class}` `` | Walks to a target after an anchor: searches the document for `private static class` first, then for `private static` *after* that anchor — so a generic target string lands on the occurrence you actually meant. Splitting is on the first colon, so neither part may contain `:`. |
...
| `` `{snip:ctor:500}` `` | Type abbreviation `ctor`, wait 500 ms, then press Tab to expand it — overrides the 200 ms default. |
| `` `{key:enter}` `` | Press a single named key. Available keys: `tab`, `enter`, `alt+enter`, `up`, `down`, `left`, `right`, `home`, `end`, `esc`. `tab` goes through the editor's action handler (smart-tab); `home` / `end` route through the editor's smart-home / smart-end actions (so e.g. Rider toggles between the first non-whitespace column and column 0 on repeated `home` presses); `esc` runs a best-effort dismiss chain; `enter` and the arrow keys are dispatched to *whatever currently has focus* — so they nudge an open popup (e.g. select the highlighted item after `` `{key:alt+enter}` ``) and otherwise route through the editor's keymap (so smart-enter still fires when no popup is up); `alt+enter` is dispatched through the IDE's full event pipeline (engaging the keymap → `ShowIntentionActions`). To move the *editor caret* instead of nudging a popup, use `` `{caret:up:N}` `` / `` `{caret:down:N}` `` / etc. |
...
| `` `{scroll:bottom:start}` `` | Arm auto-scroll: every subsequent typed character (single keystroke, multi-char insert, the `` `{key:enter}` `` macro) re-scrolls the viewport to the chosen target. Same direction set as `{scroll:DIR}` — `top`/`up`, `bottom`/`down`, `center`/`centre`. Useful when typing into a long file and you want the cursor to stay glued to one corner of the viewport. |
| `` `{scroll:end}` `` | Disarm auto-scroll. Subsequent inserts no longer move the viewport. `` `{scroll:DIR:end}` `` is accepted too (the direction is ignored — the auto-scroll state lives on the editor, not the macro). Auto-scroll also auto-resets at the start of each typing run, so a script that forgets to disarm doesn't leak across runs. |

The marker tokens (default `` `{ `` / `` }` ``) are configurable in **Settings** and update live in the macro list and the macro highlighter.

## Custom macros

Click **Edit** at the bottom of the macro list to open a popup that manages your own user-defined macros. Each entry has a **name**, an optional ordered list of **parameters**, an optional **description**, an optional **language**, and a **content** body — when the script later contains `` `{name}` `` (or `` `{name:arg1:arg2}` `` for parameterised macros), it gets substituted with that content before the typing pipeline runs. The description (when set) appears under the macro in the dialog's macro list.
...
- **Hide macro descriptions (show on hover)** — when on, macro list cells render only the syntax line and the description appears in a hover tooltip. Off by default (descriptions are inlined under each entry, the way the dialog has shipped).
