...
| `` `{key:enter}` `` | Press a single named key. Available keys: `tab`, `enter`, `alt+enter`, `up`, `down`, `left`, `right`, `esc`. Routed through the editor's action handler for `tab`/`enter`/`esc` (so smart-tab, smart-enter, and `EditorEscape`'s dismiss chain fire); `alt+enter` is dispatched through the IDE's full event pipeline (engaging the keymap → `ShowIntentionActions`); arrow keys target whatever currently has focus, suitable for nudging an open popup. To move the *editor caret* instead of nudging a popup, use `` `{caret:up:N}` `` / `` `{caret:down:N}` `` / etc. |
