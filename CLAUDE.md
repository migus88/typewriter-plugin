...
`WriteTextCommand` does **not** schedule auto-popup itself anymore — calling `scheduleAutoPopup` per-keystroke caused the popup to flicker (each schedule resets the alarm; the next char's insert then closes it via the lookup's prefix-tracker; the next schedule reopens it; repeat). The popup currently shows only at explicit `{{complete}}` sites. "Popup constantly active during typing" the way real-user typing has it would require routing letter chars through `TypedAction` (which integrates with the lookup's typed-handler chain natively) — open work, revisit if needed.
